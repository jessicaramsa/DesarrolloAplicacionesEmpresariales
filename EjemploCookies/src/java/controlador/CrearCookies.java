...
public class CrearCookies extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String ingreso = fechaHora.get(Calendar.DAY_OF_MONTH) + "/" +
                    (fechaHora.get(Calendar.MONTH) + 1) + "/" +
                    fechaHora.get(Calendar.YEAR) + "  " +
                    fechaHora.get(Calendar.HOUR) + ":" +
                    fechaHora.get(Calendar.MINUTE) + ":" +
                    fechaHora.get(Calendar.SECOND);
            
            Cookie cookieCorreo = new Cookie("correo", correo);
            Cookie cookieIngreso = new Cookie("ingreso", ingreso);
            
            response.addCookie(cookieCorreo);
            response.addCookie(cookieIngreso);
            
            response.sendRedirect("MostrarCookies");
        }
    }
